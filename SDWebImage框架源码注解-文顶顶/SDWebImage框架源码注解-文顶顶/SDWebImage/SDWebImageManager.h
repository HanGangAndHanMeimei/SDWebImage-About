...
/*
 * 如果该图片没有缓存，那么下载
 * imageManager：当前的SDWebImageManager
 * imageURL：要下载图片的URL地址
 * 返回值 如果要下载的图片在缓存中不存在，则返回NO，否则返回YES
 */
 ...
 * url 图片的URL地址
 * options 指定此次请求策略的选项
 * progressBlock 图片下载进度的回调
 * completedBlock 操作完成后的回调
 *      此参数是必须的，此block没有返回值
 ...
 *          imageURL：图片的URL地址
 * 返回值 SDWebImageOperation对象，应该是SDWebimageDownloaderOperation实例
 */
 ...
 */
//异步检查图像是否已经有内存缓存,completionBlock始终在主队列执行
 ...
 *  @note the completion block is always executed on the main queue
 */
//异步检查图像是否已经有磁盘缓存，completionBlock始终在主队列执行
