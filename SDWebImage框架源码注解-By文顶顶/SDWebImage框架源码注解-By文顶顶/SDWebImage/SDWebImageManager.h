 ...
 * 如果该图片没有缓存，那么下载
 *
 * @param imageManager The current `SDWebImageManager` 当前的SDWebImageManager
 * @param imageURL     The url of the image to be downloaded    要下载图片的URL地址
 *
 * @return Return NO to prevent the downloading of the image on cache misses. If not implemented, YES is implied. 如果要下载的图片在缓存中不存在，则返回NO，否则返回YES
 */
 ...
 * 注意：该方法在全局并发队列中执行，不是主线程
 *
 * @param imageManager The current `SDWebImageManager` 当前的SDWebImageManager
 * @param image        The image to transform   要转换你的图片
 * @param imageURL     The url of the image to transform    要转换的图片的URL地址
 *
 * @return The transformed image object.    变换后的图片对象
 */
...
// 遵守了SDWebImageManagerDelegate协议的代理
 ...
 * 返回一个SDWebImageManager的单例对象
 * @return SDWebImageManager shared instance 返回全局的 SDWebImageManager 实例(单例)
 */
 ...
 * 根据图片的URL保存图片到缓存
 *
 * @param image The image to cache 要缓存的拖
 * @param url   The URL to the image 该图片的URL
 *
 */
 ...
 *
 *  @param url image url    图片的URL
 *
 *  @return if the image was already cached 返回是否已经缓存
 */
 ...
 *  @param url image url 图片的URL
 *
 *  @return if the image was already cached (disk only) 是否存在（只检查磁盘缓存）
 */
 ...
 *
 *  @param url              image url 图片的URL
 *  @param completionBlock  the block to be executed when the check is finished 当任务执行完毕之后调用的block
 *  
 *  @note the completion block is always executed on the main queue completionBlock始终在主队列执行
 */
